if mods["Nuclear Fuel"] then
    ...
    if data.raw.recipe["nuclear-fuel-reprocessing"] then
        ...
        -- Force 1% change for balance with Bobs Plates
        if mods["bobplates"] then
            bobmods.lib.recipe.remove_result("nuclear-fuel-reprocessing", "plutonium")
            bobmods.lib.recipe.add_result("nuclear-fuel-reprocessing", {type = "item", name = "plutonium", amount = 1, probability = 0.01})
        end
    end
    ...
    if data.raw.recipe["breeder-fuel-cell"] then
        ...
        bobmods.lib.recipe.add_ingredient("breeder-fuel-cell", {type = "item", name = "uranium-238", amount = 19})
        if data.raw.item["depleted-uranium-fuel-cell"] then
